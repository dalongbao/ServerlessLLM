...
def create_app(
    ...
    async def register_handler(request: Request):
        ...
        if db.get_model(model_name, backend):
            raise HTTPException(
                status_code=409,
                detail=f"Model ({model_name}, {backend}) is already registered",
            )
        ...
        try:
            ...
            logger.info(f"Registered model ({model_name}, {backend})")
    ...
    async def delete_model_handler(model_name: str, request: Request):
        """Delete a model.
        ...
        model = db.get_model(model_name, backend)
        if not model:
            raise HTTPException(
                status_code=404,
                detail=f"Model ({model_name}, {backend}) not found",
            )
        ...
        try:
            db.update_model_status(model.model_name, model.backend, "deleting")
            db.update_desired_replicas(model.model_name, model.backend, 0)

            logger.info(
                f"Model ({model.model_name}, {model.backend}) marked for deletion"
            )
            ...
            logger.error(
                f"Failed to mark model ({model.model_name}, {model.backend}) "
                f"for deletion: {e}",
                exc_info=True,
            )
            raise HTTPException(
    ...
    async def get_models(request: Request):
        ...
        for model in models:
            ready_endpoints = (
                router.get_endpoint_count(model.model_name, model.backend)
                if router
                else 0
            )
    ...
    async def cluster_status(request: Request):
        ...
        for model in models:
            endpoints = db.get_model_endpoints(model.model_name, model.backend)
